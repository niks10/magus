...
public class ShooterGameCamera : MonoBehaviour {
	...
	public void pauseCam( bool s)
	{
		pauseCamera = s;
		//Debug.Log (pauseCamera);
	}

	public int getPauseCam()
	{
		int a = 0;
		if (pauseCamera)
			a = 1;
	...
	void LateUpdate () {
		if (Time.deltaTime == 0 || Time.timeScale == 0 || player == null || pauseCamera) {
		...
		playerModel = GameObject.Find("HarryThePlayer");
		playerModel.transform.rotation = aimRotation;
